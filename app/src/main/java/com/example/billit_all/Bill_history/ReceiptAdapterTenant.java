...
public class ReceiptAdapterTenant extends RecyclerView.Adapter<ReceiptAdapterTenant.myViewHolder> {

    protected ArrayList<Receipt> receipts;
    String billName = "Electricity";
    File dir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS);
    public ReceiptAdapterTenant (ArrayList<Receipt> receipts){
    ...
    public myViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.receipt_list_tenant,parent,false);
    ...
    public void onBindViewHolder(@NonNull myViewHolder myViewHolder, int position) {
        ...
        if (myViewHolder.statusR.getText().toString() == "PAID"){
            myViewHolder.pay_btn.setVisibility(View.INVISIBLE);

        }else {
            myViewHolder.statusR.setTextColor(Color.RED);
            myViewHolder.pay_btn.setVisibility(View.VISIBLE);
            myViewHolder.download_btn1.setVisibility(View.VISIBLE);
        }
        ...
        final int tenantID_position= receipts.get(position).getUnit_id();
        final String qrCode = receipts.get(position).getQrCode();
        final double tenantAmount_position= receipts.get(position).getAmount();
...
//                Toast.makeText(myViewHolder.pay_btn.getContext(),"this is payment acitivty",Toast.LENGTH_LONG).show();
                Intent intent = new Intent(myViewHolder.pay_btn.getContext(), PaymentElectricity.class);
                intent.putExtra("id", tenantID_position);
                intent.putExtra("amount", tenantAmount_position);
                intent.putExtra("qr", qrCode);
                myViewHolder.pay_btn.getContext().startActivity(intent);
                myViewHolder.pay_btn.setVisibility(View.INVISIBLE);
            }
        });
    ...
    public int getItemCount() {
        ...
        public class myViewHolder extends RecyclerView.ViewHolder {
            ...
            Button pay_btn;
            Button download_btn1;
            TextView date, total_consumption,total_bill, price_rate, unit_no, consumption, amount, statusR, elec_reading, water_reading,comp_total_bill, comp_total_consumption, comp_consumption, comp_price_rate, due_date, tenantID, previousTenant;
